...
- [核质分离模块设计原理说明.md](./核质分离模块设计原理说明.md)：详细解释核质分离模块的设计原理
- [训练脚本使用说明.md](./训练脚本使用说明.md)：旧版训练说明（不含核质分离模块）
- [UNet增强版使用说明.md](./UNet增强版使用说明.md)：增强版UNet架构说明

---

## 🎉 总结

增强版UNet（含核质分离感知模块）提供了灵活的训练和预测配置，可以根据需求启用或禁用各个模块。推荐使用全部模块以获得最佳性能，如果资源受限，可以灵活调整配置。
